        ...
        {items.length === 0 ? (
          ...
          <>
            <div className="mt-6 flex flex-wrap gap-2">
              <a href={shareWa} target="_blank" rel="noopener noreferrer" className="sf-btn-pine text-sm">
                WhatsApp list
              </a>
              <button
                type="button"
            ...
            <ul className="mt-8 divide-y divide-[color:var(--sf-line)] border-y border-[color:var(--sf-line)]">
              {items.map((item) => (
                <li key={item.id} className="flex gap-4 py-4">
                  <Link href={`/books/${item.slug}`} className="relative h-24 w-16 shrink-0 overflow-hidden bg-[color:var(--sf-surface)]">
                    <BookCover
